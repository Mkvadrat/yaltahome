...
class ControllerModuleFeatured extends Controller {
	protected function index($setting) {
		...
		//Модуль срочные обьекты
		$this->data['line_first'] = html_entity_decode($this->config->get('line_first'), ENT_QUOTES, 'UTF-8');
		
		$this->data['line_second'] = html_entity_decode($this->config->get('line_second'), ENT_QUOTES, 'UTF-8');
		
		$this->load->model('catalog/product'); 
		
		$this->load->model('tool/image');
		
		$latest_product = $this->model_catalog_product->getLatestProduct();
		
		$this->data['latest_product'] = array();
		
		foreach($latest_product as $latest){
			$this->data['latest_product'][] = array(
				'image' => $this->model_tool_image->resize($latest['image'], 214, 168),
				'title'  => $latest['name'],
				'name'  => utf8_substr(strip_tags(html_entity_decode($latest['name'], ENT_QUOTES, 'UTF-8')), 0, 20) . '...',
				'link'  => $this->url->link('product/product', 'product_id=' . $latest['product_id'])
			);
		}
		
		$this->data['products_lat'] = $latest_product;
		...
		if ($product_info) {
			...
			$this->data['products'][] = array(
				...
				'description' 	=> utf8_substr(strip_tags(html_entity_decode($product_info['description'], ENT_QUOTES, 'UTF-8')), 0, 114) . ' Подробнее...',
				'stock'			=> $stock,
				'minimum'		=> $minimum,
