<?php
// Heading
$_['heading_title']          = 'Категории';
$_['heading_title_category'] = 'Категория';

// Text
$_['text_success']           = 'Список категорий обновлен!';
$_['text_default']           = 'Основной магазин';
$_['text_image_manager']     = 'Менеджер изображений';
$_['text_browse']            = 'Обзор';
$_['text_clear']             = 'Очистить';

// Column
$_['column_name']            = 'Название категории';
$_['column_sort_order']      = 'Порядок сортировки';
$_['column_action']          = 'Действие';

// Entry
$_['entry_name']             = 'Название категории:';
$_['entry_meta_keyword']     = 'Мета-тег Keywords:';
$_['entry_meta_description'] = 'Мета-тег Description:';
$_['entry_top_description']  = 'Верхнее описание:';
$_['entry_description']      = 'Нижнее описание:';
$_['entry_parent']         	 = 'Родительская категория:';
$_['entry_store']            = 'Магазины:';
$_['entry_keyword']          = 'SEO URL:';
$_['entry_image']            = 'Изображение категории:';
$_['entry_maps']             = 'На карте:';
$_['entry_top']              = 'Главное меню:';
$_['entry_column']           = 'Столбцы:';
$_['entry_sort_order']       = 'Порядок сортировки:';
$_['entry_status']           = 'Статус:';
$_['entry_layout']           = 'Изменить схему:';
$_['entry_seo_title']        = 'HTML-тег Title:';
$_['entry_seo_h1']           = 'HTML-тег H1:';

// Info
$_['info_top']           	 = '<span class="help">Если флажок установлен, тогда категория будет показана в главном меню . Данную опцию нужно включать только для главных (родительских) категорий.</span>';
$_['info_column']            = '<span class="help">Укажите количество столбцов в выпадающем меню категории. Опция применяется только для главных (родительских) категорий.</span>';

// Error
$_['error_warning']          = 'Внимательно проверьте форму на ошибки!';
$_['error_permission']       = 'У Вас нет прав для изменения категорий!';
$_['error_name']             = 'Название категории должно быть от 2 до 32 символов!';
?>